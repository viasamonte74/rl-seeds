...
def downsample_depth(depth: np.ndarray, size: int) -> np.ndarray:
    """Block-pool depth to size×size×1. Must match RL/common.py.

    Env depth is 0=near, 1=far. Uniform blocks use min-pool. Mixed ground+sky
    interceptor blocks (256²) keep the nearest pixel in the far cluster so an
    airborne target is not replaced by ground.
    """
...
class InterceptorFeaturesExtractor(BaseFeaturesExtractor):
    ...
    Lives in this file so PPO.load works inside the packaged drone_agent.py.
    """
...
def _register_extractor_for_load():
    """PPO.zip pickles this class under agent_template/common/drone_agent."""
    for name in ("agent_template", "common", "drone_agent", Path(__file__).stem):
